-- ════════════════════════════════════════════════════════════════════════════
-- Feature Terceirização — vídeo institucional ou pitch personalizado
-- ════════════════════════════════════════════════════════════════════════════
-- 26/05/2026: Thales quer um vídeo rodando na landing pública.
-- Campo opcional — aceita URL YouTube, Vimeo ou MP4 direto.
-- Se vazio, a seção de vídeo simplesmente não aparece.
-- ════════════════════════════════════════════════════════════════════════════

BEGIN;

ALTER TABLE public.orcamentos
  ADD COLUMN IF NOT EXISTS terc_video_url text;

COMMENT ON COLUMN public.orcamentos.terc_video_url IS
  'URL de vídeo institucional/pitch a renderizar na landing pública. Aceita YouTube, Vimeo ou MP4 direto.';

-- Atualizar get_proposta_por_token pra retornar o novo campo
...
SELECT column_name FROM information_schema.columns
 WHERE table_schema='public' AND table_name='orcamentos' AND column_name='terc_video_url';
