-- ════════════════════════════════════════════════════════════════════════════
-- HOTFIX 27/05/2026 — RPC get_proposta_por_token retornar terc_vencimento_*
-- ════════════════════════════════════════════════════════════════════════════
-- Bug: feature-vencimento-tipo adicionou 2 colunas (terc_vencimento_tipo,
-- terc_vencimento_outros_texto) mas a RPC pública que alimenta a landing
-- usa lista EXPLÍCITA de colunas. Resultado: badge "No deferimento" não
-- aparece na landing pública (mas aparece no PDF, que lê via SELECT *).
--
-- Fix: DROP + CREATE adicionando os 2 campos no RETURNS TABLE e no SELECT.
-- ════════════════════════════════════════════════════════════════════════════
...
CREATE OR REPLACE FUNCTION public.get_proposta_por_token(p_token text, p_senha text DEFAULT ''::text)
RETURNS TABLE(
  ...
  pago_em timestamp with time zone, contrato_assinado_url text, clicksign_document_key text,
  itens_selecionados jsonb, prazo_pagamento_dias integer, empresa_id uuid, cliente_id uuid,
  created_by text, has_password boolean, tipo_proposta text, terc_modalidade text,
  terc_servicos jsonb, terc_naturezas jsonb, terc_inclusos jsonb, terc_valor_base numeric,
  terc_valor_pro numeric, terc_valor_enterprise numeric, terc_clicksign_status text,
  terc_pdf_url text, terc_observacoes_publicas text, terc_valor_final_override numeric,
  terc_precos_por_tipo jsonb, terc_regras_rapidas_ativas jsonb, terc_valor_abertura numeric,
  terc_dia_pagamento integer, terc_video_url text,
  -- 27/05: 2 campos novos
  terc_vencimento_tipo text,
...
-- Verificação
SELECT proname, pg_get_function_result(oid) ~ 'terc_vencimento_tipo' as tem_venc_tipo
FROM pg_proc
WHERE proname = 'get_proposta_por_token';
