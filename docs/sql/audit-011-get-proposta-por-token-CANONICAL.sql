-- =============================================
-- AUDIT-011 (29/05/2026) — get_proposta_por_token CANÔNICA
-- =============================================
-- Esta função foi reescrita em 8+ SQLs diferentes sem ordem documentada.
-- Este arquivo é a VERSÃO OFICIAL em produção em 29/05/2026 (capturada via
-- pg_get_functiondef). Toda mudança futura deve substituir ESTE arquivo,
-- não criar SQL novo.
--
-- SQLs históricos a IGNORAR (não rodar mais — estavam incompletos):
-- - feature-onda-conversao-27-05.sql (parte get_proposta_por_token)
-- - feature-terceirizacao-customizacao-livre.sql
-- - feature-terceirizacao-valor-abertura-dia-pagto.sql
-- - feature-terceirizacao-video-url.sql
-- - feature-terceirizacao-precos-por-tipo-regras.sql
-- - fix-auditoria-noite-27-05.sql (parte get_proposta_por_token)
-- - fix-get_proposta_por_token-vencimento-tipo.sql
-- - fix-terceirizacao-share-token-ambiguo.sql
-- - get-proposta-por-token-permite-convertido.sql
-- - notif-master-proposta-pagamento.sql (parte)
-- - sec-033-proposta-senha-real.sql (parte)
-- - sec-032-share-tokens-ttl.sql (parte)
--
-- Mantenha esses arquivos como referência histórica mas NÃO os execute.
-- =============================================

CREATE OR REPLACE FUNCTION public.get_proposta_por_token(
  p_token text,
  p_senha text DEFAULT ''::text
)
 RETURNS TABLE(
   ...
   pdf_url text, observacoes text, validade_dias integer, pagamento text,
   sla text, prazo_execucao text, ordem_execucao text, contexto text,
   ...
   created_by text, has_password boolean, tipo_proposta text,
   terc_modalidade text, terc_servicos jsonb, terc_naturezas jsonb,
   terc_inclusos jsonb, terc_valor_base numeric, terc_valor_pro numeric,
   terc_valor_enterprise numeric, terc_clicksign_status text, terc_pdf_url text,
   terc_observacoes_publicas text, terc_valor_final_override numeric,
   terc_precos_por_tipo jsonb, terc_regras_rapidas_ativas jsonb,
   terc_valor_abertura numeric, terc_dia_pagamento integer,
   terc_video_url text, terc_vencimento_tipo text,
   terc_vencimento_outros_texto text
 )
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  SELECT o.id, o.numero, o.prospect_nome, o.prospect_cnpj, o.prospect_email,
    ...
    o.terc_video_url,
    o.terc_vencimento_tipo,
    o.terc_vencimento_outros_texto
  FROM public.orcamentos o
  WHERE o.share_token = p_token
    AND o.status IN ('enviado', 'aguardando_pagamento', 'convertido', 'aceito')
    AND (o.data_expiracao IS NULL OR o.data_expiracao > NOW());
...
GRANT EXECUTE ON FUNCTION public.get_proposta_por_token(text, text) TO anon;
GRANT EXECUTE ON FUNCTION public.get_proposta_por_token(text, text) TO authenticated;

-- =============================================
-- Done. Versão canônica gravada.
-- Próxima mudança: editar ESTE arquivo, não criar novo SQL.
-- =============================================
