-- =============================================
-- Hotfix: get_proposta_por_token aceita status 'convertido'
-- =============================================
-- Bug encontrado 14/05/2026:
--   Após cliente aprovar e pagar via Asaas, o orçamento vira 'convertido'.
--   Mas a RPC filtrava status IN ('enviado', 'aguardando_pagamento'), excluindo
--   convertido → "Copiar Link" no convertido retornava "Proposta não encontrada".
--
-- Fix: inclui 'convertido' no IN. Frontend (PropostaPublica) já trata convertido
-- mostrando tela "Já aprovado" (statusFinal='aprovado'), só precisava a RPC
-- deixar passar.
--
-- Mantém: 'rascunho' e 'recusado' continuam fora.
--   - 'rascunho' não foi enviado ainda — link não deve funcionar
--   - 'recusado' fica fora por escolha de produto (cliente já recusou; reenviar
--     exige passar pra 'enviado' explicitamente).
-- =============================================
