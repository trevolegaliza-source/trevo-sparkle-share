-- ════════════════════════════════════════════════════════════════════════════
-- HOTFIX: get_proposta_por_token — column reference "share_token" is ambiguous
-- ════════════════════════════════════════════════════════════════════════════
-- Bug: ao adicionar share_token na cláusula RETURNS TABLE da nova versão
-- da função, o WHERE share_token = p_token virou ambíguo (Postgres não sabe
-- se é a coluna OUT da function ou a coluna da tabela orcamentos).
--
-- Fix: qualificar todas referências de coluna com alias da tabela (`o.`)
-- na cláusula que SELECT INTO antes do RETURN QUERY.
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- HOTFIX 25/05: qualificar com alias `o.` pra evitar ambiguidade com a
  -- coluna `share_token` declarada na cláusula RETURNS TABLE.
...
-- Verificação: deve retornar 1 linha (a proposta de teste)
SELECT id, numero, tipo_proposta, terc_modalidade, terc_valor_base
  FROM public.get_proposta_por_token('a7d5ac192a2af1fd5bc8e41aac9944c6', '');
