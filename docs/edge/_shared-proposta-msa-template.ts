// =============================================
// Template HTML — Proposta Comercial + Master Service Agreement
// =============================================
// 26/05/2026 (Fase 2 — Geração de PDF):
// Renderiza um documento HTML completo, pronto pra PDFShift converter em PDF.
// Estrutura: Anexo I (proposta de 3 páginas) + Contrato MSA (7 páginas) = 10 págs.
//
// Convenções:
//  - Tudo CSS inline (PDFShift respeita @page e print rules)
//  - Fontes: Inter via Google Fonts (embed) + fallback system-ui
//  - Tamanho: A4 portrait (210 x 297 mm)
//  - Margens via @page: 0 (gerencio via padding interno pra controlar headers/footers)
// =============================================
...
interface RegraRapida {
  id: string;
  label: string;
  texto: string;
}

const REGRAS_CATALOGO: RegraRapida[] = [
...
export interface ProposalData {
  ...
  terc_precos_por_tipo: Record<string, number> | null;
...
function valorPrincipal(p: ProposalData): number {
...
function pagCapa(p: ProposalData): string {
...
// ─── Página 2: Anexo I + II ─────────────────────────────────────────────────
function pagEscopo(p: ProposalData): string {
  const servicos = (p.terc_servicos || []).map((s) => ({ ...s, ativo: !!s.ativo }));
  const naturezas = (p.terc_naturezas || []).map((n) => ({ ...n, ativo: !!n.ativo }));
  const inclusos = p.terc_inclusos || [];
  const isPrecoPorTipo = p.terc_modalidade === 'preco_por_tipo';
  const isPlanoMensal = p.terc_modalidade === 'pro_5';

  return `
    <section class="pagina pagina-escopo">
      ...
      <div class="escopo-grid">
        <div>
          ...
          <div class="chips">
            ${servicos.map((s) => `<span class="${s.ativo ? 'chip chip-on' : 'chip chip-off'}">${escapeHtml(s.label)}</span>`).join('')}
          </div>
        </div>
        <div>
          <p class="escopo-label">Natureza Jurídica Atendida</p>
          <div class="chips">
            ${naturezas.map((n) => `<span class="${n.ativo ? 'chip chip-green' : 'chip chip-off'}">${escapeHtml(n.label)}</span>`).join('')}
          </div>
        </div>
      </div>
      ...
      ${isPrecoPorTipo ? `
        <div class="precos-tabela">
          ...
          ${Object.entries(p.terc_precos_por_tipo || {}).filter(([_, v]) => v && v > 0).map(([tipo, valor]) => `
            <div class="preco-linha">
              <span>${TIPO_PROCESSO_LABEL[tipo] || tipo}</span>
              <strong>${fmtBRL(valor as number)}</strong>
            </div>
          `).join('')}
        </div>
      ` : `
        ...
        ${p.terc_valor_abertura && p.terc_valor_abertura > 0 ? `
          <div class="valor-extras">
            <div>
              ...
              <strong>${fmtBRL(p.terc_valor_abertura)}</strong>
            </div>
            <div>
              <p>Alteração / Baixa / Transformação</p>
              <strong>${fmtBRL(valorPrincipal(p))}</strong>
            </div>
          </div>
        ` : ''}
      `}
      ...
      ${p.terc_dia_pagamento ? `
        <div class="dia-pagto">
          📅 Cobrança recorrente todo dia <strong>${p.terc_dia_pagamento}</strong> do mês.
        </div>
      ` : ''}
    </section>
  `;
...
// ─── Página 3: Condições + Vinculação ───────────────────────────────────────
function pagCondicoes(p: ProposalData): string {
  const expira = calcExpiracao(p.created_at, p.validade_dias);
  const ativas = p.terc_regras_rapidas_ativas || [];
  const regrasObj = REGRAS_CATALOGO.filter((r) => ativas.includes(r.id));

  return `
    <section class="pagina pagina-condicoes">
      <header class="pag-header">
        <p>${escapeHtml(p.prospect_nome)}</p>
      ...
      <div class="cond-grid">
        ...
        <div class="cond-box">
          ...
          <p>Prazo de início: até 5 dias úteis após recebimento COMPLETO da documentação. SLA de atendimento: 48 horas úteis.</p>
        </div>
        <div class="cond-box">
          <p class="cond-label">VALIDADE</p>
          <p>Válida por <strong>${p.validade_dias} dias</strong> a partir da data de emissão.</p>
        </div>
      </div>
      ...
      <div class="vinculacao">
        ...
        <p>
          ...
          <strong>${escapeHtml(p.prospect_nome)}</strong> e
          <strong>TREVO Assessoria Societária LTDA</strong>, incorporando por referência
          o Contrato Mestre de Prestação de Serviços (Master Service Agreement) que segue nas
          páginas seguintes deste documento. O aceite desta proposta, formal ou por execução,
          implica concordância integral com os termos.
        </p>
...
// ─── Páginas 4+: MSA ────────────────────────────────────────────────────────
function pagMSA(p: ProposalData): string {
  const modalLabel = MODALIDADE_LABEL[p.terc_modalidade] || 'Customizada';
  return `
    <section class="pagina pagina-msa-capa">
      <div class="msa-capa-content">
        ...
        <div class="msa-rodape">
          <p>Trevo Legaliza · CEO Thales Felipe Burger Soares</p>
        </div>
      </div>
    </section>

    <section class="pagina pagina-msa-partes">
      ...
      <p>1.1 O presente instrumento tem por objeto a prestação de serviços técnicos especializados em assessoria societária, legalização empresarial e atos correlatos, conforme detalhamento constante no Anexo I — Proposta Comercial, que integra o presente contrato para todos os fins de direito.</p>
      <p>1.2 A modalidade contratada será: <strong>${modalLabel}</strong>, conforme expressamente indicado na proposta aceita.</p>
      <p>1.3 O escopo limita-se estritamente aos serviços descritos no Anexo I, não se presumindo qualquer obrigação adicional não expressamente pactuada.</p>

      <h3 class="clausula-titulo">02 DO ONBOARDING E INÍCIO DA PARCERIA</h3>
      <p>2.1 Em até 2 (dois) dias úteis após a assinatura deste instrumento, a CONTRATANTE receberá acesso à Plataforma Trevo e o checklist completo de documentação necessária ao início dos processos.</p>
    ...
    <section class="pagina pagina-msa-clausulas">
      <header class="pag-header">
        <p>${escapeHtml(p.prospect_nome)}</p>
        <p class="muted">Master Service Agreement — Cláusulas Contratuais</p>
      </header>
      ...
      <p>4.1 O início da contagem de qualquer prazo contratual está condicionado ao recebimento integral da documentação solicitada. Para facilitar esse processo, a CONTRATADA disponibiliza checklist detalhado antes do início de cada processo.</p>
      <p>4.2 A eventual antecipação de etapas por liberalidade da CONTRATADA não implica início formal de prazo.</p>
      <p>4.3 A inércia da CONTRATANTE por período superior a 30 (trinta) dias, quanto ao envio de documentos ou informações indispensáveis, autoriza a suspensão ou encerramento da execução, sem restituição de valores já pagos.</p>
      ...
      <p>5.2 A CONTRATADA não se responsabiliza por indeferimentos, exigências ou atrasos decorrentes de: I — análises discricionárias de órgãos públicos; II — instabilidades sistêmicas governamentais; III — alterações normativas supervenientes; IV — informações inexatas ou incompletas fornecidas pela CONTRATANTE; V — ausência ou irregularidade de certificados digitais.</p>
    ...
    <section class="pagina pagina-msa-clausulas">
      ...
      <p>9.1 Demandas com prazo inferior a 24 (vinte e quatro) horas caracterizam regime de urgência e poderão sofrer acréscimo de até 50% (cinquenta por cento) sobre os honorários contratados, além das taxas de registro aplicáveis.</p>
      ...
      <p>11.2 O descumprimento desta cláusula sujeitará o infrator ao pagamento de multa compensatória de R$ 20.000,00 (vinte mil reais), sem prejuízo de apuração de perdas e danos adicionais.</p>
    </section>
    ...
    <section class="pagina pagina-msa-assinatura">
      ...
      <p class="msa-rodape-pdf">
        Este instrumento é gerado automaticamente pela plataforma TREVO ENGINE e integra o conjunto
        documental composto pela Proposta Comercial (Anexo I) e Condições Financeiras (Anexo II). O
        conjunto completo constitui o acordo entre as partes.
      </p>
    </section>
  `;
}
...
/* ─── Capa ─── */
.pagina-capa {
  background: linear-gradient(135deg, #022c22 0%, #052e16 60%, #0f172a 100%);
  color: #ecfdf5;
  display: flex;
...
.capa-badge {
  display: inline-block; margin-top: 10mm; padding: 6px 14px; background: #047857;
  color: white; font-size: 9px; font-weight: 700; letter-spacing: 0.1em; border-radius: 4px;
}
...
.capa-stats {
  ...
  padding: 8mm 0; border-top: 1px solid rgba(255,255,255,0.1);
}
...
.capa-footer {
  display: flex; justify-content: space-between; padding-top: 6mm;
  border-top: 1px solid rgba(255,255,255,0.1); font-size: 9px; color: rgba(255,255,255,0.6);
}
.capa-footer .mono { font-family: monospace; }

/* ─── Páginas Anexo / Condições ─── */
.pag-header {
  margin-bottom: 10mm; padding-bottom: 4mm; border-bottom: 1px solid #e2e8f0;
  font-size: 10px;
}
...
.sec-title {
  ...
  letter-spacing: 0.1em; padding: 6px 10px; margin-bottom: 8mm;
}
...
.clausula-titulo {
  background: #0f172a; color: white; font-size: 9px; font-weight: 700;
  letter-spacing: 0.08em; padding: 5px 10px; margin: 5mm 0 3mm;
}
...
export function renderProposalHTML(p: ProposalData): string {
  return `<!DOCTYPE html>
<html lang="pt-BR">
<head>
  <meta charset="UTF-8" />
  <title>Proposta Comercial — ${escapeHtml(p.prospect_nome)}</title>
  <style>${CSS}</style>
</head>
<body>
  ${pagCapa(p)}
