...
const CURVA_SVG = `<svg class="curva-bg" viewBox="0 0 400 1188" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
  <defs>
  ...
  <path d="M 280 0 Q 360 300 320 594 Q 280 888 360 1188 L 400 1188 L 400 0 Z" fill="url(#curveGrad)"/>
  <path d="M 300 0 Q 380 300 340 594 Q 300 888 380 1188" stroke="#ffffff" stroke-width="0.4" fill="none" opacity="0.15"/>
</svg>`;
...
/* PALETA OFICIAL — alinhada ao template MSA
   Verde fundo:         #1e3a2a
   Verde fundo darker:  #142519
   Verde quase preto:   #0a1f12
   Verde acento:        #84cc16  (lima brilhante)
   Verde médio:         #65a30d
   Verde água:          #a8c4af
   Off-white:           #e8e8e8
*/
...
.pagina {
  ...
  overflow: hidden;
  background: #1e3a2a;
  /* Textura granulada sutil (replica MSA) */
  background-image:
    repeating-linear-gradient(135deg, transparent 0px, transparent 38px, rgba(255,255,255,0.012) 38px, rgba(255,255,255,0.012) 40px),
    repeating-linear-gradient(45deg, transparent 0px, transparent 38px, rgba(255,255,255,0.008) 38px, rgba(255,255,255,0.008) 40px);
}
.pagina:last-child { page-break-after: auto; }

/* Curva decorativa SVG no canto direito */
.curva-bg {
  position: absolute;
  top: 0;
  right: -30mm;
...
.pag-divisor {
  height: 1px;
  margin: 0 16mm;
  background: linear-gradient(90deg, transparent 0%, rgba(132,204,22,0.5) 30%, rgba(132,204,22,0.5) 70%, transparent 100%);
  position: relative;
  z-index: 2;
}
...
.capa-cliente-card {
  ...
  border: 1px solid rgba(168,196,175,0.15);
...
.capa-cliente-meta span { display: inline-flex; align-items: center; gap: 5px; }
.capa-cliente-meta .ic { color: #84cc16; }
...
.capa-headline-wrap {
  margin-top: 8mm;
  padding-top: 6mm;
  border-top: 1px solid rgba(168,196,175,0.12);
}
.capa-headline {
  font-family: 'Cinzel', serif;
  font-size: 24px;
...
.capa-stats {
  ...
  gap: 4mm;
  border-top: 1px solid rgba(168,196,175,0.15);
  border-bottom: 1px solid rgba(168,196,175,0.15);
}
...
.capa-cards-titulo::before,
.capa-cards-titulo::after { content: '— '; opacity: 0.6; }
.capa-cards-titulo::after { content: ' —'; }
...
.capa-card {
  padding: 5mm 4mm;
  background: rgba(255,255,255,0.04);
  border: 1px solid rgba(168,196,175,0.12);
...
.capa-footer-warn { color: #fbbf24; font-weight: 600; }
.capa-footer-prop {
  font-family: 'Cinzel', serif;
  color: white;
...
.chip-group {
  ...
  border: 1px solid rgba(168,196,175,0.1);
  border-left: 3px solid #84cc16;
...
.chip-on { background: #84cc16; color: #0a1f12; }
.chip-off {
  background: transparent;
  color: #6b7d72;
  text-decoration: line-through;
  border: 1px solid rgba(168,196,175,0.18);
}
...
.incluso-off {
  ...
  border: 1px solid rgba(168,196,175,0.08);
  border-left: 3px solid rgba(168,196,175,0.15);
  opacity: 0.5;
}
.incluso-icon {
  width: 16px;
...
.incluso .incluso-icon { background: #84cc16; color: #0a1f12; }
.incluso-off .incluso-icon { background: rgba(168,196,175,0.2); color: #a8c4af; }
.incluso-nome {
  font-family: 'Cinzel', serif;
  font-size: 11px;
  font-weight: 600;
...
.incluso-off .incluso-nome { text-decoration: line-through; color: #a8c4af; }
.incluso-desc {
  font-size: 9px;
  color: #a8c4af;
  line-height: 1.5;
...
.invest-card {
  padding: 12mm 8mm;
  background: linear-gradient(135deg, #142519 0%, #0a1f12 60%, #142519 100%);
...
.invest-extras {
  margin-top: 6mm;
  padding-top: 4mm;
  border-top: 1px solid rgba(168,196,175,0.18);
  font-size: 10px;
  color: #a8c4af;
}
.invest-extras strong { color: white; font-weight: 600; }

.cond-stack { display: flex; flex-direction: column; gap: 3mm; }
.cond-item {
  background: rgba(255,255,255,0.04);
  border: 1px solid rgba(168,196,175,0.12);
  border-left: 3px solid #84cc16;
...
.cond-item p strong { color: white; font-weight: 600; }
...
.dia-pagto strong { color: #fbbf24; font-weight: 700; }
...
.pag-num-footer::before,
.pag-num-footer::after { content: '— '; }
.pag-num-footer::after { content: ' —'; }
`;
...
function renderPropostaHTML(p: any): string {
  ...
  const valorTotal = valorPrincipal(p);
  const propNum = `PROP-${fmtDataCurta(new Date(p.created_at)).replace(/\//g, '-')}`;
  const valorLabel = isPlanoMensal ? 'Investimento Mensal' : 'Honorários do Processo';
  const valorDesc = isPlanoMensal ? '5 processos inclusos/mês com 15% de desconto' : 'Valor fixo por processo / operação societária';
  const logoSrc = `data:image/png;base64,${LOGO_TREVO_PNG_B64}`;

  return `<!DOCTYPE html>
<html lang="pt-BR">
<head><meta charset="UTF-8"/><style>${CSS_PROPOSTA}</style></head>
<body>
...
<section class="pagina">
  ...
  <header class="pag-header">
    <img src="${logoSrc}" class="pag-header-logo" alt="Trevo Legaliza"/>
    <div class="pag-header-info">
      <div><strong>TREVO ASSESSORIA SOCIETÁRIA LTDA</strong></div>
      <div>CNPJ 39.969.412/0001-70 · Atuação Nacional</div>
    </div>
  </header>
  <div class="pag-divisor"></div>

  <div class="capa-content">
    <p class="capa-eyebrow">Proposta Comercial</p>
    <h1 class="capa-titulo">Departamento <em>societário</em><br/>especializado para seu escritório</h1>

    <div class="capa-cliente-card">
      <p class="capa-cliente-label">Preparada exclusivamente para</p>
      <p class="capa-cliente-nome">${escapeHtml(p.prospect_nome).toUpperCase()}</p>
      <div class="capa-cliente-meta">
        ${p.prospect_cnpj ? `<span><span class="ic">▸</span> CNPJ ${escapeHtml(p.prospect_cnpj)}</span>` : ''}
        ${p.prospect_telefone ? `<span><span class="ic">▸</span> ${escapeHtml(p.prospect_telefone)}</span>` : ''}
        ${p.prospect_email ? `<span><span class="ic">▸</span> ${escapeHtml(p.prospect_email)}</span>` : ''}
      </div>
    </div>

    <span class="capa-modalidade">▸ ${modalLabel.toUpperCase()}</span>

    <div class="capa-headline-wrap">
      <h2 class="capa-headline">Seu escritório contábil cresceu.<br/><em>Sua estrutura societária ainda te acompanha?</em></h2>
      <p class="capa-descricao">Atendemos escritórios contábeis de todos os portes — desde estruturas enxutas até operações de grande volume — com processos padronizados, previsibilidade operacional e alta capacidade de execução. Do início ao deferimento, com SLA formalizado e rastreabilidade integral.</p>
    </div>

    <div class="capa-stats">
      <div class="capa-stat"><div class="capa-stat-num">12</div><div class="capa-stat-desc">anos de<br/>expertise societária</div></div>
      <div class="capa-stat"><div class="capa-stat-num">26</div><div class="capa-stat-desc">estados de<br/>atuação ativa</div></div>
      <div class="capa-stat"><div class="capa-stat-num">∞</div><div class="capa-stat-desc">parcerias com<br/>Juntas Comerciais</div></div>
      <div class="capa-stat"><div class="capa-stat-num">B2B</div><div class="capa-stat-desc">exclusivo<br/>para contabilidades</div></div>
    </div>

    <h3 class="capa-cards-titulo">Por que a Trevo é a escolha óbvia</h3>
    <div class="capa-cards">
      <div class="capa-card">
        <p class="capa-card-titulo">Você para de perder tempo</p>
        <p class="capa-card-desc">Não gerencie processos societários sem controle. Nós assumimos, executamos e entregamos dentro do SLA — você foca no seu cliente.</p>
      </div>
      <div class="capa-card">
        <p class="capa-card-titulo">Qualquer estado, mesmo padrão</p>
        <p class="capa-card-desc">Atendemos 26 estados. Seu cliente em São Paulo ou no Pará recebe o mesmo nível de execução e rastreabilidade.</p>
      </div>
      <div class="capa-card">
        <p class="capa-card-titulo">Zero surpresa financeira</p>
        <p class="capa-card-desc">Taxas, emolumentos e custos extras são informados antes da execução. Sem cobrança surpresa para você.</p>
      </div>
      <div class="capa-card">
        <p class="capa-card-titulo">Estruturado para escalar</p>
        <p class="capa-card-desc">Do plano básico ao enterprise, o modelo acompanha o crescimento do seu escritório.</p>
      </div>
    </div>
  </div>

  <div class="capa-footer">
    <span>Reconhecida nacionalmente · 12 anos de mercado · @trevolegaliza</span>
    <span><span class="capa-footer-warn">⚠ Expira em ${fmtDataCurta(expira)}</span> &nbsp; <span class="capa-footer-prop">${propNum}</span></span>
  </div>
</section>
...
<section class="pagina">
  ...
  <header class="pag-header">
    <img src="${logoSrc}" class="pag-header-logo" alt="Trevo Legaliza"/>
    <div class="pag-header-info">
      <div><strong>${escapeHtml(p.prospect_nome).toUpperCase()}</strong></div>
      ${p.prospect_cnpj ? `<div>${escapeHtml(p.prospect_cnpj)}</div>` : ''}
    </div>
  </header>
  <div class="pag-divisor"></div>

  <div class="interna-content">
    <p class="interna-eyebrow">Anexo I · Escopo da Proposta</p>
    <h2 class="interna-titulo">Detalhamento <em style="font-style: italic; color: #84cc16;">Técnico</em></h2>

    <div class="bloco-2col">
      <div>
        <p class="bloco-eyebrow">Natureza Jurídica</p>
        <div class="chip-group"><div class="chips">
          ${naturezas.map((n: any) => `<span class="${n.ativo ? 'chip chip-on' : 'chip chip-off'}">${escapeHtml(n.label)}</span>`).join('')}
        </div></div>
      </div>
      <div>
        <p class="bloco-eyebrow">Serviços</p>
        <div class="chip-group"><div class="chips">
          ${servicos.map((s: any) => `<span class="${s.ativo ? 'chip chip-on' : 'chip chip-off'}">${escapeHtml(s.label)}</span>`).join('')}
        </div></div>
      </div>
    </div>

    <p class="bloco-eyebrow">Escopo Técnico Incluso</p>
    <div class="inclusos">
      ${inclusos.map((i: any) => `
        <div class="${i.ativo ? 'incluso' : 'incluso-off'}">
          <span class="incluso-icon">${i.ativo ? '✓' : '✕'}</span>
          <div>
            <p class="incluso-nome">${escapeHtml(i.label)}</p>
    ...
    ${regrasObj.length > 0 || p.terc_observacoes_publicas ? `
      <p class="bloco-eyebrow">Observações &amp; Condições</p>
      <div class="obs-box">
        ${regrasObj.map((r) => `<p>▸ ${escapeHtml(r.texto)}</p>`).join('')}
        ${p.terc_observacoes_publicas ? `<p style="margin-top:3mm;padding-top:3mm;border-top:1px dashed rgba(251,191,36,0.3)">▸ ${escapeHtml(p.terc_observacoes_publicas).replace(/\n/g, '<br>')}</p>` : ''}
      </div>
    ` : ''}
  </div>

  <div class="pag-num-footer">Página 2 de 3</div>
</section>

<!-- PÁGINA 3 — CONDIÇÕES FINANCEIRAS -->
<section class="pagina">
  ${CURVA_SVG}
  <header class="pag-header">
    <img src="${logoSrc}" class="pag-header-logo" alt="Trevo Legaliza"/>
    <div class="pag-header-info">
      <div><strong>${escapeHtml(p.prospect_nome).toUpperCase()}</strong></div>
      <div>Condições Comerciais</div>
    </div>
  </header>
  <div class="pag-divisor"></div>

  <div class="interna-content">
    <p class="interna-eyebrow">Anexo II · Investimento</p>
    <h2 class="interna-titulo">Condições <em style="font-style: italic; color: #84cc16;">Financeiras</em></h2>

    <div class="invest-grid">
      <div class="invest-card">
        <p class="invest-eyebrow">${valorLabel}</p>
        <p class="invest-valor"><span class="invest-valor-prefix">R$</span>${fmtBRLnoSym(valorTotal)}</p>
        <p class="invest-desc">${valorDesc}</p>
        ${p.terc_valor_abertura && p.terc_valor_abertura > 0 ? `<div class="invest-extras">Abertura de empresa: <strong>${fmtBRL(p.terc_valor_abertura)}</strong></div>` : ''}
      </div>
      <div class="cond-stack">
        <div class="cond-item">
          <p class="cond-label">Pagamento</p>
          <p>Cobrança via boleto bancário em até 3 dias da data da solicitação. Pagamento à vista.</p>
        </div>
        <div class="cond-item">
          <p class="cond-label">SLA &amp; Prazos</p>
          <p>Prazo para início: até 5 dias úteis após recebimento <strong>COMPLETO</strong> da documentação. SLA de atendimento: 48 horas úteis.</p>
        </div>
        <div class="cond-item">
          <p class="cond-label">Validade</p>
          <p>Válida por <strong>${p.validade_dias || 15} dias</strong> a partir da emissão.</p>
        </div>
      </div>
    </div>

    ${p.terc_dia_pagamento && isPlanoMensal ? `<div class="dia-pagto">▸ Cobrança recorrente todo dia <strong>${p.terc_dia_pagamento}</strong> do mês.</div>` : ''}

    <div class="vinc-box">
      <p class="vinc-titulo">Vinculação Contratual</p>
      <p class="vinc-texto">A presente Proposta Comercial é parte integrante do relacionamento jurídico entre <strong>${escapeHtml(p.prospect_nome).toUpperCase()}</strong> e a <strong>TREVO Assessoria Societária LTDA</strong>. O aceite implica concordância integral com os termos, escopo técnico, limites de responsabilidade e regras operacionais. Condições de prazo e SLA aplicáveis após recebimento da documentação.</p>
      <div class="vinc-rodape">
        <span class="vinc-data">São Paulo, ${dataHoje}</span>
        <span class="badge-vinc">● Documento Vinculante</span>
      </div>
    </div>

    <p class="disclaimer">Ao aceitar esta proposta, a CONTRATANTE declara ciência e acordo com todas as especificações descritas. Esta proposta não constitui contrato, sendo necessária a assinatura do Contrato Mestre para formalização do vínculo.</p>
  </div>

  <div class="pag-num-footer">Página 3 de 3</div>
</section>
...
async function gerarPropostaPdf(orc: any): Promise<Uint8Array> {
  const html = renderPropostaHTML(orc);
