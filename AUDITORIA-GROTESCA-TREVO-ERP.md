...
| **SEC-023** | ✅ FIXADO | Botão master "Resetar 2FA" em Gestão de Usuários (item no dropdown de cada user, exceto self/master) + AlertDialog de confirmação. Edge function nova `resetar-2fa-usuario` lista todos os fatores TOTP do target via admin API e deleta um por um. Pós-reset, o próximo login dele cai em forceSetup (SEC-021). **Pendente deploy manual da edge function:** `supabase functions deploy resetar-2fa-usuario`. |
| **SEC-024** | ✅ FIXADO | Recovery codes só pra master (8 códigos one-time, formato `XXXX-XXXX-XXXX` em alfabeto sem ambiguidade). Tabela `mfa_recovery_codes` (RLS: user lê só os próprios; escrita só service_role). Edge functions `gerar-recovery-codes` (master-only) e `verify-recovery-code` (atomic update + delete dos fatores TOTP do user). UI: `RecoveryCodesCard` em `/configuracoes → Segurança` com gerar/copiar/baixar/imprimir e checkbox obrigatório "salvei em local seguro" pra fechar modal. `MfaChallenge` agora tem link "Não tenho meu celular → usar código de recuperação". **Pendente:** rodar SQL em `docs/sql/sec-024-mfa-recovery-codes.sql` no SQL Editor + deploy `gerar-recovery-codes` e `verify-recovery-code`. |
| **SEC-025** | ✅ FIXADO | Alerta de login novo IP/device. Tabela `login_history` (RLS: só master da empresa lê). Edge function `registrar-login` é chamada no `SIGNED_IN` do `AuthContext`, registra IP + /24 + user-agent + país (CF-IPCountry quando presente) e compara com últimos 30 dias. Se subnet ou UA novo → notificação tipo `login_novo` pra empresa (filtro client-side SEC-019 garante que só master vê). IP exibido truncado (`.x` no último octeto). Primeiro login do user nunca dispara alerta. **Pendente:** rodar `docs/sql/sec-025-login-history.sql` + deploy `registrar-login`. |
| **SEC-026** | ✅ FIXADO | `TrocarSenhaCard` agora pede **senha atual** antes de trocar (anti session-hijack — alguém pega navegador aberto e troca a senha em 5s). Revalida via `signInWithPassword` (rate-limited pelo Supabase). Indicador visual fraca/média/forte enquanto digita. |
| **SEC-027** | ✅ FIXADO | Validação de força em todos os 5 pontos de criação/troca de senha (`TrocarSenhaCard`, `ResetPassword`, `handleDefinirSenha`, `handleCriarComSenha`, edge functions `definir-senha-usuario` e `criar-usuario-com-senha`). Regras: ≥10 chars + letra + número + bloqueio de top-60 senhas comuns (12345678, password, qwerty, senha123, trevo2026 etc). Util compartilhado em `src/lib/password-validator.ts` + espelho Deno em `_shared/password-validator.ts`. **Pendente deploy** das 2 edge functions atualizadas. |
